...
const type = generateActionType(INVOICES_STATE_FEATURE_NAME, 'Editor - Adjustments update');

export class InvoicesEditorAdjustmentsUpdateAction implements BaseAction<InvoicesState> {
  ...
  handler(state: InvoicesState, action: this): InvoicesState {
    const editor = setStateProperties(state.editor, {
      ...
      invoice: calcDocTotals(state.editor.invoice, state.editor.products, action.payload),
    });

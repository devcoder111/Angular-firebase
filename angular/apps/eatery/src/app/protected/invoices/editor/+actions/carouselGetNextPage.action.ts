...
const type = generateActionType(INVOICES_STATE_FEATURE_NAME, 'Editor - Carousel get next page');

export class CarouselGetNextPageAction implements BaseAction<InvoicesState> {
  ...
  handler(state: InvoicesState, action: this): InvoicesState {
    const editor = setStateProperties(state.editor, {
      carousel: {
        currentItem: state.editor.carousel.currentItem + 1,
